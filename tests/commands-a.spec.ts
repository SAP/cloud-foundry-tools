...
describe("commands unit tests", () => {
    ...
    describe("cmdSelectAndSaveTarget", () => {
        ...
        it("ok:: save current target operation selected", async () => {
            ...
            cfLocalMock.expects("cfGetTarget").resolves(target);
            //...cmdCreateTarget
            vscodeWindowMock.expects("showInputBox").withExactArgs({ prompt: messages.name_for_target, ignoreFocusOut: true }).resolves(label);
        ...
        it("ok:: save current target operation selected, create target command canceled", async () => {
            ...
            vscodeWindowMock.expects("showInputBox").withExactArgs({ prompt: messages.name_for_target, ignoreFocusOut: true }).resolves();
            //
            expect(await commands.cmdSelectAndSaveTarget()).to.be.undefined;
        });
        ...
        it("ok:: save current target operation selected, target setup is incompleted, canceled", async () => {
            ...
            vscodeWindowMock.expects("showWarningMessage").withExactArgs(messages.target_setup_not_completed(t.org, t.space), 'OK', 'Cancel').resolves('Cancel');
            expect(await commands.cmdSelectAndSaveTarget()).to.be.undefined;
        });
        ...
        it("ok:: save current target operation selected, target setup is incompleted (partial), continued", async () => {
            ...
            cfLocalMock.expects("cfGetTarget").resolves(t);
            vscodeWindowMock.expects("showWarningMessage").withExactArgs(messages.target_setup_not_completed(t.org, t.space), 'OK', 'Cancel').resolves('OK');
            //...cmdCreateTarget canceled
            vscodeWindowMock.expects("showInputBox").withExactArgs({ prompt: messages.name_for_target, ignoreFocusOut: true }).resolves();
            //
            expect(await commands.cmdSelectAndSaveTarget()).to.be.undefined;
        ...
        it("ok:: save current target operation selected, create target command throws exception", async () => {
            ...
            //
            vscodeWindowMock.expects("showErrorMessage").withExactArgs(toText(new Error(cli.stdout))).resolves();
            expect(await commands.cmdSelectAndSaveTarget()).to.be.undefined;
        });
        ...
        it("ok:: new target operation selected, other endPoint typed", async () => {
            ...
            vscodeWindowMock.expects("showInputBox").withExactArgs({ prompt: messages.enter_cf_endpoint, value: target["api endpoint"], ignoreFocusOut: true }).resolves(selectedEndPoint);
        ...
        it("ok:: new target operation selected, endPoint selection canceled", async () => {
            ...
            vscodeWindowMock.expects("showQuickPick").resolves({ id: 'pick-save' });
            cfLocalMock.expects("cfGetTarget").resolves(target);
            vscodeWindowMock.expects("showInputBox").withExactArgs({ prompt: messages.enter_cf_endpoint, value: target["api endpoint"], ignoreFocusOut: true }).resolves();
            expect(await commands.cmdSelectAndSaveTarget()).to.be.undefined;
        });
        ...
        it("ok:: new target operation selected, same endPoint typed", async () => {
            ...
            vscodeWindowMock.expects("showQuickPick").resolves({ id: 'pick-save' });
            cfLocalMock.expects("cfGetTarget").resolves(target);
            vscodeWindowMock.expects("showInputBox").withExactArgs({ prompt: messages.enter_cf_endpoint, value: target["api endpoint"], ignoreFocusOut: true }).resolves(target["api endpoint"]);
            // ...cmdCFSetOrgSpace
            vscodeWindowMock.expects("withProgress").withArgs({ location: nsVsMock.testVscode.ProgressLocation.Notification, title: messages.verify_cf_connectivity, cancellable: false }).resolves({ data: {} });
            vscodeWindowMock.expects("withProgress").withArgs({ location: nsVsMock.testVscode.ProgressLocation.Window, title: messages.getting_orgs }).resolves(orgs);
            vscodeWindowMock.expects("showQuickPick").withExactArgs(orgs, { placeHolder: messages.select_org, canPickMany: false, matchOnDetail: true, ignoreFocusOut: true }).resolves(orgs[1]);
